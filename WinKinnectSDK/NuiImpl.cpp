...
#pragma warning(disable: 4127) // conditional expression is constant
    ...
    while(1)
    {
        ...
        if( dt > 1000 )
        {
			...
			// @@@ : REMPLACER INFO POUR AFFICHER UNE VARIABLE INT
            ::PostMessageW(m_hWnd, WM_USER_UPDATE_FPS, IDC_FPS, info);
        }
...
void CSkeletalViewerApp::Nui_GotVideoAlert( )
{
	...
	// pImageFrame est du type (eImageType) NUI_IMAGE_TYPE_COLOR (rgb32)

    INuiFrameTexture * pTexture = pImageFrame->pFrameTexture;
    NUI_LOCKED_RECT LockedRect;
    pTexture->LockRect( 0, &LockedRect, NULL, 0 );
	// LockedRect.Pitch est la longueur de chaque ligne (640*4=2560)
    if( LockedRect.Pitch != 0 )
    {
		...
		// Pour construire une image (la première vue par la Kinect)
		//creerPpm("extrait.ppm", pBuffer);
		//exit(0);

		// @@@ : MODIF !!!!!
		//info = sizeof(pBuffer) / sizeof(BYTE);
		//info = (int) pBuffer[4];
		/*for (int ligne = 0; ligne < 480; ligne++) {
			for (int colonne = 0; colonne < 640; colonne++) {
				for (int fois = 0; fois < 4; fois++) {
					if (indice < 614400) {
						pBuffer[indice] = 0;
					} else {
						pBuffer[indice] = 255;
